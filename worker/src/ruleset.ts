import { ClientTarget, Env } from './types.js';
...
export async function handleRuleset(
  ...
  const cacheKey = `ruleset:${target}:${rulesetUrl}`;
  if (!force && env.CACHE) {
    const cached = await env.CACHE.get(cacheKey);
    if (cached) return rulesetResponse(cached);
  }

  const raw = await cachedFetch(env.CACHE, rulesetUrl, force);
  if (!raw) return new Response('Failed to fetch upstream ruleset', { status: 502 });

  const converted = convertRulesetContent(raw, target);

  if (env.CACHE) {
    await env.CACHE.put(cacheKey, converted, { expirationTtl: 86400 });
  }
