...
// Rule-provider URLs point back at this service's own /ruleset/{idx} endpoint
// (not the upstream GitHub URL) so mihomo/Clash clients never reach GitHub either.

function generateRuleProviders(rulesets: RuleSet[], selfBase: string): [string[], Map<RuleSet, string>] {
  ...
  for (const rs of rulesets) {
    ...
    lines.push(
      `  ${candidate}:`, '    type: http', `    url: ${quote(`${selfBase}/ruleset/${idx}?t=shadowrocket`)}`,
...
export async function generateClash(env: Env, selfBase: string, force = false): Promise<string> {
  ...
  const [ruleProviderLines, providerNames] = generateRuleProviders(rulesets, selfBase);
